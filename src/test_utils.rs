...
pub(crate) fn copy_git_object_file(hash: &str) -> std::io::Result<()> {
    let (subdirectory, file_name) =
        crate::object::Object::get_path_from_hash(hash).expect("Invalid hash");

    let from = PathBuf::from(".git/objects")
        .join(subdirectory)
        .join(file_name);
    let to = PathBuf::from(".hamachi/objects")
        .join(subdirectory)
        .join(file_name);
